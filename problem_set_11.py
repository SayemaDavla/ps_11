...
def board_passengers(starship, passengers):
    ...
    max_passengers = starship.get("max_passengers", 0)
    starship["passengers_on_board"] = passengers[:max_passengers]
    return starship
...
def create_person(data, planets=None):
    ...
    # Convert homeworld to dictionary representation
    homeworld = None
    if planets:
        ...
        swapi_homeworld = get_swapi_resource(person.get("homeworld"))
        if swapi_homeworld:
            homeworld = create_planet(swapi_homeworld)
...
def create_planet(data):
    ...
    planet = utl.convert_none_values(data, ("N/A", "unknown"))
    ...
    return {
        ...
        "suns": utl.convert_to_int(planet.get("suns")),
        'moons': utl.convert_to_int(planet.get("moons")),
        "orbital_period_days": utl.convert_to_float(planet.get("orbital_period")),
        "diameter_km": utl.convert_to_int(planet.get("diameter")),
        "gravity_std": convert_gravity_value(planet.get("gravity")),
        "climate": utl.convert_to_list(planet.get("climate", ":")),
        "terrain": utl.convert_to_list(planet.get("terrain", ":")),
        "population": utl.convert_to_int(planet.get("population"))
    }
...
def create_starship(data):
    ...
    Returns:
        dict: new dictionary.
    """
    starship = utl.convert_none_values(data)
    starship["length_m"] = utl.convert_to_float(starship.get("length"))
    starship["max_atmosphering_speed"] = utl.convert_to_int(starship.get ("max_atmosphering_speed"))
    starship["hyperdrive_rating"] = utl.convert_to_float(starship.get("hyperdrive_rating"))
    starship["top_speed_mglt"] = utl.convert_to_int(starship.get("MGLT"))
    starship["crew_size"] = utl.convert_to_int(starship.get("crew"))
    starship["max_passengers"] = utl.convert_to_int(starship.get("passengers"))
    starship["armament"] = utl.convert_to_list(starship.get("armament"))
    starship["cargo_capacity_kg"] = utl.convert_to_int(starship.get("cargo_capacity"))
...
def create_vehicle(data):
    ...
    vehicle = utl.convert_none_values(data)
    vehicle["length_m"] = utl.convert_to_float(vehicle.get("length"))
    vehicle["max_atmosphering_speed"] = utl.convert_to_int(vehicle.get("max_atmosphering_speed"))
    vehicle["hyperdrive_rating"] = utl.convert_to_float(vehicle.get("hyperdrive_rating"))
    vehicle["crew_size"] = utl.convert_to_int(vehicle.get("crew"))
    vehicle["max_passengers"] = utl.convert_to_int(vehicle.get("passengers"))
    vehicle["armament"] = utl.convert_to_list(vehicle.get("armament"))
    vehicle["cargo_capacity_kg"] = utl.convert_to_int(vehicle.get("cargo_capacity"))
...
def get_mandalorian_data(mandalorian_data, filter):
    ...
    """
    for item in mandalorian_data:
        if item['name'].lower() == filter.lower():
            return item
    return None
...
def main():
    ...
    # TODO call function
    utl.write_json(mandalorian_nevarro, 'stu-mandalorian_nevarro.json')
    utl.write_json(mandalorian_arvala_7, 'stu-mandalorian_arvala_7.json')


    # Problem 3.5-3.6  Test < create_planet >
    # TODO call function
    swapi_tatooine = get_swapi_resource(utl.SWAPI_PLANETS_URL, {'search': 'Tatooine'}).json()['results'][0]
    tatooine = create_planet(swapi_tatooine)

    # Problem 3.7 Write to file
    # TODO call function
    utl.write_json(tatooine, 'stu-tatooine.json')
